...
// The radio uses pin 10 (the other slave select pin on the Arduino) so use
// pin 4 for FPGA communication
const int FPGA_SS = 4;
...
unsigned int pack_payload(unsigned char x, unsigned char y) {
...
void setup(void)
{
  ...
  //
  // Setup and configure FPGA SPI link
  //
  ...
  if ( role == role_pong_back )
  {
    ...
    if ( radio.available() )
    {
      ...
      while (!done)
      {
        ...
        printf("Got payload %.2x...", got_data & 0xff);

        // Send it to the payload to the FPGA
        // It expect the same packet format as the radio uses
        digitalFPGAWrite(got_data);
