...
* Debug! Now is a great time to find all of the error cases. For example, does the robot always detect walls independent of where they are? Try different maze setups and show that it still works. Does it always start independent of where the speaker is located? Does it always ignore decoys, and always stop for other robots? Even if you can't solve all of these problems in time for the lab hand-in, it is important to locate them and allocate time to deal with them before your system becomes more complicated. 
